...
class FlowDataset(data.Dataset):
    def __init__(self, aug_params=None, sparse=False, eval_points=False):
        self.augmentor = None
        self.sparse = sparse
...
class Easytear(FlowDataset):
    def __init__(self, aug_params=None, split='training', root='datasets/Easytear'):
        super(Easytear, self).__init__(aug_params, eval_points=True)
        if split == 'testing':
            self.is_test = True

        xml_path = osp.join(root, "annotations.xml")
        root = osp.join(root, split)

        xml_root = ET.parse(xml_path).getroot()
        tracks = xml_root.findall("track")
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    elif args.stage == 'easytear':
        train_dataset = Easytear(split="training")
